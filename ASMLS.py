...
def sliverLS(nSliver=25, vrb=False, MaxChi=1.5, nBoots=1000, nMinPerStrip=50, DoPlot=False, nsim=200, changePer=True):
    ...
    ##nsim is the number of TKLC sims to run
    ...
    for j in range(nSliver):
        ...
        ThisPeak, PeakPow=FindPeakPeriod(vPer, vPowers, vrb=vrb, DoPlot=DoPlot)
        ...
        ##KCD June 18 2017: no longer standard deviations here.
        #size is always 1000
        #want a 1000x25 array
        #im_err= np.zeros((25,1000))
        #KCD June 30 2017: remove bootstrap calls
        BootPer=0
        #BootPer[j]=BootstrapTrials(vtime[g][gStrip], vrate[g][gStrip], nBoots)
        #sim_err=np.zeros((200, 25))
        #outerr=np.zeros(nsim)
        p=0

        ##based off of suggested code in email from Will July 2 2017:

        mytrials=np.zeros(nsim)
        ...
        while p <nsim:
            ...
            mytrials[p]=tklcsim(vtime[g][gStrip], vrate[g][gStrip], verr[g][gStrip], j,  ThisPeak, DoPlot=False, avgper=changePer)
            p+=1
        
        if np.size(sim_err)<1:
            sim_err=mytrials
        else:
            sim_err=np.vstack((sim_err,mytrials))
    ...
    SliverDict={'Powers':vArr, 'Periods':1.0/vFreqs,\
                        'TStart':vBegin, 'TEnd':vEnd,'boots':BootPer, 'peaks':PeakPeriods, 'peakpowers':PeakPow,'simerr': sim_err}

    sFil='StackedPowers.pickle'
    if changePer:
        sFil='StackedPowers_avgper.pickle'

    pickle.dump(SliverDict, open(sFil, 'w'))
...
def FindPeakPeriod(vPeriods, vPowers, vrb=False, DoPlot=False):
...
def BootstrapTrials(vTimes, vRates, nTrials):
    ...
    for i in range(nTrials):
        ...
        thisPeak, apow=FindPeakPeriod(vPer, vPowers)
    ...
    ##KCD June 18 2017: Changing this so that it returns array of periods
    stDev=np.std(fPer[ind_good])

    #print stDev
    #return stDev
    
    print fPer[ind_good].size, "argelfraster"
    return fPer[ind_good]
...
def tklcsim(time, flux, flux_err, inte, peakperiod,DoPlot=False, InjectPeak=True, nSliver=25, MaxChi=1.5, nMinPerStrip=50, avgper=True):
    ...
    if InjectPeak:
        if avgper:
            peakperiod=55.0
        peaksignal=3.3*np.sin((2.0*np.pi*tklc.time)/peakperiod)
    ...
    if InjectPeak:
        ...
        ThisPeak, PeakPow=FindPeakPeriod(1.0/Freq, Sim_LS)
